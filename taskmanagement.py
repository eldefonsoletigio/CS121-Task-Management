...
class MainWindow:
    ...
    def addtask(self):
        try:
            ...
            # Add record to database and get task ID
            task_id = Database_TaskManagement.addRec(entertask, startdate, enddate, category, priority, status)

            # Clear input fields
            self.clearData()
            self.CategoryEntry.set('')  # Clear Category dropdown
            self.PriorityEntry.set('')  # Clear Priority dropdown
            self.StatusEntry.set('')    # Clear Status dropdown
    ...
    def deleteData(self):
        try:
            ...
            task_id = self.taskTree.item(selected_item)["values"][0]  # Get task ID from the first column
    ...
    def displayData(self):
        try:
            ...
            # Insert new task data into the table
            for task in tasks:
    ...
    def hideData(self):
        # Remove all data from the tree view (hides the data)
    ...
    def updateData(self):
        try:
            selected_item = self.taskTree.selection()[0]
            task_id = self.taskTree.item(selected_item)["values"][0]  # Assuming the first column contains the task ID
            entertask = self.EnterTaskEntry.get()
            startdate = self.EnterDateEntry.get()
            enddate = self.EnterEndDateEntry.get()
            category = self.CategoryEntry.get()
            ...
            # Show a popup indicating that the data was successfully updated
